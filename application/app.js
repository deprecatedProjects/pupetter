...
const second = async () => {
    ...
    // const strongs = await page.$$('model-viewer')
    const strongs = page.evaluate(() => {
        
        return new Promise((resolve, reject) => {
            resolve(document.getElementsByTagName("model-viewer").addEventListener('load', ()=>{console.log("load")}));
        });
    })
    // strongs[0].addEventListener('load', ()=>{console.log("sss");});
    // console.log(strongs);
    ...
    strongs.then((modelViewer) => {
        console.log("log");
        // modelViewer.addEventListener('load', ()=>{console.log("sss");});
    })
    try {
        // await page.waitFor(20000);

        await page.screenshot({ path: 'Ex2.png', fullPage: true });
